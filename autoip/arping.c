...
#define PACKAGE_VERSION "20211215-11-g2410ed1"
...
int main( int argc, char **argv ){
    ...
    if( ctl.device.name == NULL || *ctl.device.name == '\0' ){
        error( 1, 0, "Please use -I specify interface" );
    }

    if (inet_aton(ctl.target, &ctl.gdst) == 0) {
        error( 1, errno, "not regular ipv4 address" );
    }
